...
for epoch in range(num_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if torch.isnan(loss).any() or torch.isinf(loss).any():
            ...
            loss = loss.detach()
            del loss
            del output, prob_batch, prob_batch2, pos_prob, neg_prob, pos_prob2
        ...
        if (i + 1) % accumulation_steps == 0 or (i + 1) == len(dataloader):
            gradient_l2_norm = torch.norm(
                torch.cat([param.grad.view(-1) for param in auroracap.llm.parameters() if param.grad is not None])
            )
            num_grad_params = sum(
                param.grad.numel() for param in auroracap.llm.parameters() if param.grad is not None
            )
            ...
            avg_logs = {
                ...
                "pos_prob": round(sum(log["pos_prob"] for log in batch_logs) / len(batch_logs) ,8),
                "neg_prob": round(sum(log["neg_prob"] for log in batch_logs) / len(batch_logs) ,8),
    ...
    auroracap.llm.save_pretrained(f'{model_dir}/epoch{epoch+1}')